/***************************************************************************//**
 *   @file   plugin.c
 *   @brief  MAX11205 Plugin for libadnoos (AUTO-GENERATED)
 *   @author libadnoos
********************************************************************************
...
#include "drivers/adc/max11205/max11205.h"
...
{
	struct max11205_init_param *param = (struct max11205_init_param *)
		no_os_calloc(1, sizeof(struct max11205_init_param));
	if (!param)
		return NULL;
...
int32_t adnoos_init_device(void **device, void *init_param)
{
	...
	struct max11205_init_param *param = (struct max11205_init_param *)init_param;
	struct max11205_dev *dev = NULL;
	
	int32_t ret = max11205_init(&dev, param);
	if (ret != 0) {
		return ret;
	}
...
int32_t adnoos_remove_device(void *device)
{
	...
	struct max11205_dev *dev = (struct max11205_dev *)device;
	return max11205_remove(dev);
}
