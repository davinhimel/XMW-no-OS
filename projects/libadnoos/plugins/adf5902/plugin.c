/***************************************************************************//**
 *   @file   plugin.c
 *   @brief  ADF5902 Plugin for libadnoos (AUTO-GENERATED)
 *   @author libadnoos
********************************************************************************
...
#include "drivers/frequency/adf5902/adf5902.h"
...
{
	struct adf5902_init_param *param = (struct adf5902_init_param *)
		no_os_calloc(1, sizeof(struct adf5902_init_param));
	if (!param)
		return NULL;
...
int32_t adnoos_init_device(void **device, void *init_param)
{
	...
	struct adf5902_init_param *param = (struct adf5902_init_param *)init_param;
	struct adf5902_dev *dev = NULL;
	
	int32_t ret = adf5902_init(&dev, param);
	if (ret != 0) {
		return ret;
	}
...
int32_t adnoos_remove_device(void *device)
{
	...
	struct adf5902_dev *dev = (struct adf5902_dev *)device;
	return adf5902_remove(dev);
}
