/***************************************************************************//**
 *   @file   plugin.c
 *   @brief  ADMFM2000 Plugin for libadnoos (AUTO-GENERATED)
 *   @author libadnoos
********************************************************************************
...
#include "drivers/frequency/admfm2000/admfm2000.h"
...
{
	struct admfm2000_init_param *param = (struct admfm2000_init_param *)
		no_os_calloc(1, sizeof(struct admfm2000_init_param));
	if (!param)
		return NULL;
...
int32_t adnoos_init_device(void **device, void *init_param)
{
	...
	struct admfm2000_init_param *param = (struct admfm2000_init_param *)init_param;
	struct admfm2000_dev *dev = NULL;
	
	int32_t ret = admfm2000_init(&dev, param);
	if (ret != 0) {
		return ret;
	}
...
int32_t adnoos_remove_device(void *device)
{
	...
	struct admfm2000_dev *dev = (struct admfm2000_dev *)device;
	return admfm2000_remove(dev);
}
