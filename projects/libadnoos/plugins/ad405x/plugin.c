/***************************************************************************//**
 *   @file   plugin.c
 *   @brief  AD405X Plugin for libadnoos (AUTO-GENERATED)
 *   @author libadnoos
********************************************************************************
...
#include "drivers/adc/ad405x/ad405x.h"
...
{
	struct ad405x_init_param *param = (struct ad405x_init_param *)
		no_os_calloc(1, sizeof(struct ad405x_init_param));
	if (!param)
		return NULL;
...
int32_t adnoos_init_device(void **device, void *init_param)
{
	...
	struct ad405x_init_param *param = (struct ad405x_init_param *)init_param;
	struct ad405x_dev *dev = NULL;
	
	int32_t ret = ad405x_init(&dev, param);
	if (ret != 0) {
		return ret;
	}
...
int32_t adnoos_remove_device(void *device)
{
	...
	struct ad405x_dev *dev = (struct ad405x_dev *)device;
	return ad405x_remove(dev);
}
