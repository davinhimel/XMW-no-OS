/***************************************************************************//**
 *   @file   plugin.c
 *   @brief  ADT75 Plugin for libadnoos (AUTO-GENERATED)
 *   @author libadnoos
********************************************************************************
...
#include "drivers/temperature/adt75/adt75.h"
...
{
	struct adt75_init_param *param = (struct adt75_init_param *)
		no_os_calloc(1, sizeof(struct adt75_init_param));
	if (!param)
		return NULL;
...
int32_t adnoos_init_device(void **device, void *init_param)
{
	...
	struct adt75_init_param *param = (struct adt75_init_param *)init_param;
	struct adt75_dev *dev = NULL;
	
	int32_t ret = adt75_init(&dev, param);
	if (ret != 0) {
		return ret;
	}
...
int32_t adnoos_remove_device(void *device)
{
	...
	struct adt75_dev *dev = (struct adt75_dev *)device;
	return adt75_remove(dev);
}
