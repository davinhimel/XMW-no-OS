/***************************************************************************//**
 *   @file   plugin.c
 *   @brief  MAX22017 Plugin for libadnoos (AUTO-GENERATED)
 *   @author libadnoos
********************************************************************************
...
#include "drivers/dac/max22017/max22017.h"
...
{
	struct max22017_gpio_init_param *param = (struct max22017_gpio_init_param *)
		no_os_calloc(1, sizeof(struct max22017_gpio_init_param));
	if (!param)
		return NULL;
...
int32_t adnoos_init_device(void **device, void *init_param)
{
	...
	struct max22017_gpio_init_param *param = (struct max22017_gpio_init_param *)init_param;
	struct max22017_dev *dev = NULL;
	
	int32_t ret = max22017_init(&dev, param);
	if (ret != 0) {
		return ret;
	}
...
int32_t adnoos_remove_device(void *device)
{
	...
	struct max22017_dev *dev = (struct max22017_dev *)device;
	return max22017_gpio_remove(dev);
}
