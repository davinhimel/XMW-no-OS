/***************************************************************************//**
 *   @file   plugin.c
 *   @brief  AD9081 Plugin for libadnoos (AUTO-GENERATED)
 *   @author libadnoos
********************************************************************************
...
#include "drivers/adc/ad9081/ad9081.h"
...
{
	struct link_init_param *param = (struct link_init_param *)
		no_os_calloc(1, sizeof(struct link_init_param));
	if (!param)
		return NULL;
...
int32_t adnoos_init_device(void **device, void *init_param)
{
	...
	struct link_init_param *param = (struct link_init_param *)init_param;
	struct ad9081_dev *dev = NULL;
	
	int32_t ret = ad9081_jesd204_link_init(&dev, param);
	if (ret != 0) {
		return ret;
	}
...
int32_t adnoos_remove_device(void *device)
{
	...
	struct ad9081_dev *dev = (struct ad9081_dev *)device;
	return ad9081_remove(dev);
}
