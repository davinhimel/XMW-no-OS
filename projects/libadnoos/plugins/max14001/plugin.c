/***************************************************************************//**
 *   @file   plugin.c
 *   @brief  MAX14001 Plugin for libadnoos (AUTO-GENERATED)
 *   @author libadnoos
********************************************************************************
...
#include "drivers/adc/max14001/max14001.h"
...
{
	struct max14001_init_param *param = (struct max14001_init_param *)
		no_os_calloc(1, sizeof(struct max14001_init_param));
	if (!param)
		return NULL;
...
int32_t adnoos_init_device(void **device, void *init_param)
{
	...
	struct max14001_init_param *param = (struct max14001_init_param *)init_param;
	struct max14001_dev *dev = NULL;
	
	int32_t ret = max14001_init(&dev, param);
	if (ret != 0) {
		return ret;
	}
...
int32_t adnoos_remove_device(void *device)
{
	...
	struct max14001_dev *dev = (struct max14001_dev *)device;
	return max14001_remove(dev);
}
