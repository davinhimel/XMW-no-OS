/***************************************************************************//**
 *   @file   plugin.c
 *   @brief  MAX538X Plugin for libadnoos (AUTO-GENERATED)
 *   @author libadnoos
********************************************************************************
...
#include "drivers/dac/max538x/max538x.h"
...
{
	struct max538x_init_param *param = (struct max538x_init_param *)
		no_os_calloc(1, sizeof(struct max538x_init_param));
	if (!param)
		return NULL;
...
int32_t adnoos_init_device(void **device, void *init_param)
{
	...
	struct max538x_init_param *param = (struct max538x_init_param *)init_param;
	struct max538x_dev *dev = NULL;
	
	int32_t ret = max538x_init(&dev, param);
	if (ret != 0) {
		return ret;
	}
...
int32_t adnoos_remove_device(void *device)
{
	...
	struct max538x_dev *dev = (struct max538x_dev *)device;
	return max538x_remove(dev);
}
