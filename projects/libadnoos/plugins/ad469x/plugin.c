/***************************************************************************//**
 *   @file   plugin.c
 *   @brief  AD469X Plugin for libadnoos (AUTO-GENERATED)
 *   @author libadnoos
********************************************************************************
...
#include "drivers/adc/ad469x/ad469x.h"
...
{
	struct ad469x_init_param *param = (struct ad469x_init_param *)
		no_os_calloc(1, sizeof(struct ad469x_init_param));
	if (!param)
		return NULL;
...
int32_t adnoos_init_device(void **device, void *init_param)
{
	...
	struct ad469x_init_param *param = (struct ad469x_init_param *)init_param;
	struct ad469x_dev *dev = NULL;
	
	int32_t ret = ad469x_init(&dev, param);
	if (ret != 0) {
		return ret;
	}
...
int32_t adnoos_remove_device(void *device)
{
	...
	struct ad469x_dev *dev = (struct ad469x_dev *)device;
	return ad469x_remove(dev);
}
