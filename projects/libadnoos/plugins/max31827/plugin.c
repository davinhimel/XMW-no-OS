/***************************************************************************//**
 *   @file   plugin.c
 *   @brief  MAX31827 Plugin for libadnoos (AUTO-GENERATED)
 *   @author libadnoos
********************************************************************************
...
#include "drivers/temperature/max31827/max31827.h"
...
{
	struct max31827_init_param *param = (struct max31827_init_param *)
		no_os_calloc(1, sizeof(struct max31827_init_param));
	if (!param)
		return NULL;
...
int32_t adnoos_init_device(void **device, void *init_param)
{
	...
	struct max31827_init_param *param = (struct max31827_init_param *)init_param;
	struct max31827_dev *dev = NULL;
	
	int32_t ret = max31827_init(&dev, param);
	if (ret != 0) {
		return ret;
	}
...
int32_t adnoos_remove_device(void *device)
{
	...
	struct max31827_dev *dev = (struct max31827_dev *)device;
	return max31827_remove(dev);
}
