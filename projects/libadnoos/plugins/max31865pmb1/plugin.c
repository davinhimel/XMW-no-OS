/***************************************************************************//**
 *   @file   plugin.c
 *   @brief  MAX31865PMB1 Plugin for libadnoos (AUTO-GENERATED)
 *   @author libadnoos
********************************************************************************
...
#include "drivers/temperature/max31865pmb1/max31865.h"
...
{
	struct max31865_init_param *param = (struct max31865_init_param *)
		no_os_calloc(1, sizeof(struct max31865_init_param));
	if (!param)
		return NULL;
...
int32_t adnoos_init_device(void **device, void *init_param)
{
	...
	struct max31865_init_param *param = (struct max31865_init_param *)init_param;
	struct max31865_dev *dev = NULL;
	
	int32_t ret = max31865_init(&dev, param);
	if (ret != 0) {
		return ret;
	}
...
int32_t adnoos_remove_device(void *device)
{
	...
	struct max31865_dev *dev = (struct max31865_dev *)device;
	return max31865_remove(dev);
}
