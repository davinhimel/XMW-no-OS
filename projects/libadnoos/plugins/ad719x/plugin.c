/***************************************************************************//**
 *   @file   plugin.c
 *   @brief  AD719X Plugin for libadnoos (AUTO-GENERATED)
 *   @author libadnoos
********************************************************************************
...
#include "drivers/adc/ad719x/ad719x.h"
...
{
	struct ad719x_init_param *param = (struct ad719x_init_param *)
		no_os_calloc(1, sizeof(struct ad719x_init_param));
	if (!param)
		return NULL;
...
int32_t adnoos_init_device(void **device, void *init_param)
{
	...
	struct ad719x_init_param *param = (struct ad719x_init_param *)init_param;
	struct ad719x_dev *dev = NULL;
	
	int32_t ret = ad719x_init(&dev, param);
	if (ret != 0) {
		return ret;
	}
...
int32_t adnoos_remove_device(void *device)
{
	...
	struct ad719x_dev *dev = (struct ad719x_dev *)device;
	return ad719x_remove(dev);
}
