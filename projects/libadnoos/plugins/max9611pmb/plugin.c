/***************************************************************************//**
 *   @file   plugin.c
 *   @brief  MAX9611PMB Plugin for libadnoos (AUTO-GENERATED)
 *   @author libadnoos
********************************************************************************
...
#include "drivers/adc/max9611pmb/max9611.h"
...
{
	struct max9611_init_param *param = (struct max9611_init_param *)
		no_os_calloc(1, sizeof(struct max9611_init_param));
	if (!param)
		return NULL;
...
int32_t adnoos_init_device(void **device, void *init_param)
{
	...
	struct max9611_init_param *param = (struct max9611_init_param *)init_param;
	struct max9611_dev *dev = NULL;
	
	int32_t ret = max9611_init(&dev, param);
	if (ret != 0) {
		return ret;
	}
...
int32_t adnoos_remove_device(void *device)
{
	...
	struct max9611_dev *dev = (struct max9611_dev *)device;
	return max9611_remove(dev);
}
