/***************************************************************************//**
 *   @file   plugin.c
 *   @brief  LTC268X Plugin for libadnoos (AUTO-GENERATED)
 *   @author libadnoos
********************************************************************************
...
#include "drivers/dac/ltc268x/ltc268x.h"
...
{
	struct ltc268x_init_param *param = (struct ltc268x_init_param *)
		no_os_calloc(1, sizeof(struct ltc268x_init_param));
	if (!param)
		return NULL;
...
int32_t adnoos_init_device(void **device, void *init_param)
{
	...
	struct ltc268x_init_param *param = (struct ltc268x_init_param *)init_param;
	struct ltc268x_dev *dev = NULL;
	
	int32_t ret = ltc268x_init(&dev, param);
	if (ret != 0) {
		return ret;
	}
...
int32_t adnoos_remove_device(void *device)
{
	...
	struct ltc268x_dev *dev = (struct ltc268x_dev *)device;
	return ltc268x_remove(dev);
}
